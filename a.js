...
let opensonge =async function () {
    ...
    if (this.children.length == 1) {
        st = Date.now();
        getmessage(await work({ lastbackgroundImage: Date.now(), i: this.id, file: fils[this.id], st, open: true }));
    } else {
...
let getmessage = function (msg) {
    ...
    if (msg.d != '') {
        ...
        img.src = "https://mohamed-ayman-aly.github.io/musicplayer/a.png";
    }
...
async function work(mes) {
    const { i, file, open, lastbackgroundImage } = mes;
    ...
    try {
      debugger
        ...
        return{ d: '', i, format: '', open, lastbackgroundImage };
    }
};